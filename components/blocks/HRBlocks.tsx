...
import { Empregado, PeriodoRemuneracao, ServidorCedido } from '../../types';
            ...
            renderItem={(emp, idx, update) => (
                <div className="space-y-4">
                    ...
                    <div className="bg-slate-50 p-3 rounded border border-slate-200">
                        ...
                        {emp.periodos_remuneracao?.map((p: PeriodoRemuneracao, pIdx: number) => (
            ...
            renderItem={(item, i, update) => (
                <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <Field label="Data Final Cessão (Opcional)" type="date" value={item.data_final_cessao || ''} onChange={(v:any)=>update('data_final_cessao',v)} />
                    ...
                    <div className="bg-slate-50 p-3 rounded">
                         <div className="flex justify-between items-center mb-2">
                            <h5 className="text-[10px] font-bold uppercase text-slate-500">Holerites (Opcional se ônus na origem)</h5>
                            <button 
                                onClick={() => update('periodos_cessao', [...(item.periodos_cessao||[]), { mes: 1, carga_horaria: 0, remuneracao_bruta: 0 }])} 
                                className="text-[10px] font-bold text-blue-600"
                            >
                                + Mês
                            </button>
                         </div>
                         {(item.periodos_cessao || []).map((p, pIdx) => (
                             <div key={pIdx} className="flex gap-2 mb-1">
                                 <input className={CLS.INPUT + " w-16 py-1 h-8 text-xs"} type="number" placeholder="Mês" value={p.mes} onChange={e => {
                                     const arr = [...item.periodos_cessao];
                                     arr[pIdx].mes = Number(e.target.value);
                                     update('periodos_cessao', arr);
                                 }} />
                                 <input className={CLS.INPUT + " w-20 py-1 h-8 text-xs"} type="number" placeholder="C. Horária" value={p.carga_horaria} onChange={e => {
                                     const arr = [...item.periodos_cessao];
                                     arr[pIdx].carga_horaria = Number(e.target.value);
                                     update('periodos_cessao', arr);
                                 }} />
                                 <input className={CLS.INPUT + " w-full py-1 h-8 text-xs"} type="number" placeholder="Valor (R$)" value={p.remuneracao_bruta} onChange={e => {
                                     const arr = [...item.periodos_cessao];
                                     arr[pIdx].remuneracao_bruta = Number(e.target.value);
                                     update('periodos_cessao', arr);
                                 }} />
                                 <button onClick={() => {
                                     const arr = [...item.periodos_cessao];
